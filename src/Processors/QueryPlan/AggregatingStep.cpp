...
AggregatingStep::AggregatingStep(
    ...
    : ITransformingStep(
          input_header_,
          std::make_shared<const Block>(appendGroupingColumn(
              params_.getHeader(*input_header_, final_), params_.keys, !grouping_sets_params_.empty(), group_by_use_nulls_)),
...
void AggregatingStep::transformPipeline(QueryPipelineBuilder & pipeline, const BuildQueryPipelineSettings & settings)
{
    ...
    if (!grouping_sets_params.empty())
    {
        ...
        pipeline.transform([&](OutputPortRawPtrs ports)
        {
            ...
            for (size_t set_counter = 0; set_counter < grouping_sets_size; ++set_counter)
            {
                ...
                auto dag = makeCreatingMissingKeysForGroupingSetDAG(
                    *header, output_header, grouping_sets_params, set_counter, group_by_use_nulls);
    ...
    else
    {
        ...
        pipeline.resize(
            should_produce_results_in_order_of_bucket_number ? 1 : params.max_threads,
            false,
...
std::unique_ptr<AggregatingProjectionStep> AggregatingStep::convertToAggregatingProjection(const SharedHeader & input_header) const
{
    ...
    auto aggregating_projection = std::make_unique<AggregatingProjectionStep>(
        SharedHeaders{input_headers.front(), input_header}, params, final, merge_threads, temporary_data_merge_threads);
...
{
    output_header = std::make_shared<const Block>(appendGroupingColumn(
        params.getHeader(*input_headers.front(), final), params.keys, !grouping_sets_params.empty(), group_by_use_nulls));
}
...
AggregatingProjectionStep::AggregatingProjectionStep(
    SharedHeaders input_headers_, Aggregator::Params params_, bool final_, size_t merge_threads_, size_t temporary_data_merge_threads_)
    : params(std::move(params_))
    , final(final_)
    , merge_threads(merge_threads_)
    , temporary_data_merge_threads(temporary_data_merge_threads_)
